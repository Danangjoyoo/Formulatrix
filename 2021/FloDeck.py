...
driver = KvaserFmlxDriver(device_addr)
d = FmlxDevice(driver, device_addr, path_deck)
d.connect()
...
connect_to("d",device_addr,path_deck)
...
travel_velocity = 80 * eng_value #gravimetric
travel_acceleration = 150 * eng_value #gravimetric
travel_jerk = 20*eng_value #gravimetric
#travel_velocity = 200 * eng_value #platereader
#travel_acceleration = 430 * eng_value #platereader
#travel_jerk = 75*eng_value #platereader
...
home_acceleration = 150 * eng_value
...
def move_motor_abs(id,pos,wait=True,estop=False,warn=True):
    global move_done
...
zero_deck = [[265,103],[163.5,103.5],[62.2,103.5]]
...
Rack_coord_0 = dict(zip(wellname, Points0.tolist()))
Rack_coord_1 = dict(zip(wellname, Points1.tolist()))
Rack_coord_2 = dict(zip(wellname, Points2.tolist()))
...
def setTravelMode(mode):
    global travel_velocity, travel_acceleration, travel_jerk
    if mode == 'gravimetric' or mode == 'grav':
        travel_velocity = 80 * eng_value #gravimetric
        travel_acceleration = 150 * eng_value #gravimetric
        travel_jerk = 20*eng_value #gravimetric
    elif mode == 'platereader' or mode == 'plate':
        travel_velocity = 230 * eng_value #platereader
        travel_acceleration = 480 * eng_value #platereader
        travel_jerk = 90*eng_value #platereader

#zero mode SETTER
def setZeroDeckMode(mode):
    global zero_deck, row0,row1,row2,col0,col1,col2,Rack_coord_0, Rack_coord_1,Rack_coord_2,racks
    if mode == 20:
        zero_deck = [[265,102.5],[163.5,103.5],[62.2,103.5]] # UNTUK CADDY P20
    elif mode == 200:
