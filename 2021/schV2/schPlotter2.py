...
SPlotter --> Shadow Plotter
+ High Speed (0.1 - 20 ms/tick)
+ Safe! It runs on different GIL (multiprocessing)
- Manual setUp, you have to reset your config manually
...
class CPlotter():
	...
	def liveplot(self,*s,**show):
		...
		if self.staticVar:
			for var in self.staticVar: self.staticVar[var].append(self.wiplot.plot(pen=tuple(np.random.randint(50,255,3)), name=var))
	...
	def addStaticChart(self,key,val,scale=1.0,offset=0.0,copyScaling=None):
		scale, offset = 3, 4
		if copyScaling:
			self.staticVar[key+'(static)'] = [[], val,  True, self.varPack[copyScaling][scale], self.varPack[copyScaling][offset]]
		else:
