...
def align_wlld(rack, well, target=-40, depth=-100, evade=-40):
    ...
    z = math.ceil(c.p.get_motor_pos(0) / 100);
    LLD.zero = z
    print
    '\tDLLT:', dllt, '| Z :', z, '\r'
    c.clear_estop()
...
def sensor_check():
    ...
    P1_avg = True if DefaultSensorVal.P1_ - ToleranceSensorVal.Tol_P <= val[
        0] <= DefaultSensorVal.P1_ + ToleranceSensorVal.Tol_P else False
    P2_avg = True if DefaultSensorVal.P2_ - ToleranceSensorVal.Tol_P <= val[
        1] <= DefaultSensorVal.P2_ + ToleranceSensorVal.Tol_P else False
    Col_avg = True if DefaultSensorVal.Col_ - ToleranceSensorVal.Tol_Col <= val[
        2] <= DefaultSensorVal.Col_ + ToleranceSensorVal.Tol_Col else False
    Res_avg = True if DefaultSensorVal.Res_210 - ToleranceSensorVal.Tol_Res <= val[
        3] <= DefaultSensorVal.Res_210 + ToleranceSensorVal.Tol_Res else False
    Br_avg = True if DefaultSensorVal.Br - ToleranceSensorVal.Tol_Br <= val[
        4] <= DefaultSensorVal.Br + ToleranceSensorVal.Tol_Br else False
    ...
    if status:
        ...
        'P1\t|P2\t|Col\t|Res\t|Br'
        print
        str(val[0]) + '\t|' + str(val[1]) + '\t|' + str(val[2]) + '\t|' + str(val[3]) + '\t|' + str(val[4])
...
def resistance_check():
    ...
    st_2000 = True if DefaultSensorVal.Res_2000 - ToleranceSensorVal.Tol_Res <= val[
        0] <= DefaultSensorVal.Res_2000 + ToleranceSensorVal.Tol_Res and val[
                          1] < ToleranceSensorVal.Tol_dev_R2000 else False
    st_510 = True if DefaultSensorVal.Res_510 - ToleranceSensorVal.Tol_Res <= val[
        2] <= DefaultSensorVal.Res_510 + ToleranceSensorVal.Tol_Res and val[
                         3] < ToleranceSensorVal.Tol_dev_R510 else False
    st_120 = True if DefaultSensorVal.Res_120 - ToleranceSensorVal.Tol_Res <= val[
        4] <= DefaultSensorVal.Res_120 + ToleranceSensorVal.Tol_Res and val[
                         5] < ToleranceSensorVal.Tol_dev_R120 else False
...
def phase_1():
    ...
    print
    'Phase 1 done, result: ', phase_1_status
...
def phase_2():
    ...
    print
    'Phase 2 done, result: ', phase_2_status
...
def menu():
    ...
    'menu()\t: show functions '
    print
    'phase_1()\t: 1st phase MFQC'
    print
    'phase_2()\t: 2nd phase MFQC'
    print
    'phase_3()\t: 2nd phase MFQC'
    print
    'sensor_check()'
    print
    'resistance_check()'
...
def phase_3():
    ...
    c.eject()
    print
    'Put P200 tip in 0,A1'
    raw_input('Enter to continue')
    align(0, 'A1')
    picktip()
    align(0, 'A1')
    print
    'Put SBS with water in rack 1, A12'
    raw_input('Enter to continue')
    dllt_linearity()
    PLLD_response()
...
class avoidInpErr():
    ...
    def pauseFunct(message):
        i = 0;
        avoidInpErr.pauseMessage = message
        while avoidInpErr.pauseStat:
            if i == 0:
                print
                "###################### OPERATION PAUSED! CTRL+SHIFT+P TO UNPAUSE ######################"
            i += 1
...
def wait_nline():
    ...
    while True:
        ...
        "i => ", i
        if i == timeout:
            val = avoidInpErr.reInput("TIMEOUT!!  \n >> input timbangan:  ", avoidInpErr.test_inputTimbangan,
                                      warning=True)  # avoid error from unintended pressed enter
            val = float(val)
            return val, (time.time() - startStable) * 1000
            print
            "TO - val use manual input"
            break
...
def gravimetric(vol):
    ...
    after_asp = berat_now
    print
    "Weight after ASP", after_asp

    ASP_Weight = abs(after_asp - before_asp)
    print
    "Aspirate weight =", ASP_Weight
    ...
    print
    "Weight after DISP", after_disp

    DISP_Weight = abs(after_disp - after_asp)
    print
    "Dispense weight =", DISP_Weight
...
class AsyncWrite(threading.Thread):
    ...
    def run(self):
        ...
        # time.sleep(2)
        print
        "Finished background file write to" + str(self.out) + '\r',
...
def nimbang_1ml(*args):
    ...
    if proceed:
        ...
        "   ====== !! CAUTION !! =====   "
        print
        "Don't set zeroing on gravimetric! Make sure the weight is postive!"
        print
        "This is used to measure the initial and final weight"
        co = avoidInpErr.reInput("Have you setup the gravimetric? y/n ", warning=True);
        co = str.lower(co)
        while True:
            if co == 'y':
                break
            else:
                co = avoidInpErr.reInput("Have you setup the gravimetric? y/n ");
                co = str.lower(co)
        ...
        filename = 'grav\Gravimetric vol_' + str(vol) + '_' + time.strftime('%Y%m%d%H%M%S',
        ...
        for x in range(iter):
            ...
            if picktipstat[0]:
                ...
                print
                "Weight - vol:\t", weight_asp, calcvol
                print
                "Pip time", pipetting_time
                # push_data(liquid+",P"+str(tip)+",asp,"+str(vol)+","+str(weight_asp)+","+str(density)+","+str(calcvol),filename)
                string_pushData += dates + "," + str(temp) + "," + liquid + ",P" + str(
                    tip) + ",asp," + operation + "," + str(vol) + "," + str(weight_asp) + "," + str(
                    density) + "," + str(calcvol) + "," + str(stableTime) + "," + str(pipetting_time) + "," + str(
                    Half_Cycle_Time_ms)
                ...
                if x == iter - 1:
                    ...
                    finalWeight = weight2
                    string_pushData += dates + "," + str(temp) + "," + liquid + ",P" + str(
                        tip) + ",dsp," + operation + "," + str(vol) + "," + str(weight_dsp) + "," + str(
                        density) + "," + str(calcvol) + "," + str(stableTime) + "," + str(pipetting_time) + "," + str(
                        Half_Cycle_Time_ms) + ',' + str(Full_Cycle_time_ms)
                else:
                    string_pushData += dates + "," + str(temp) + "," + liquid + ",P" + str(
                        tip) + ",dsp," + operation + "," + str(vol) + "," + str(weight_dsp) + "," + str(
                        density) + "," + str(calcvol) + "," + str(stableTime) + "," + str(pipetting_time) + "," + str(
                ...
                print
                'Cycle Time >>', [saved_cycleTime[1] - saved_cycleTime[0], saved_cycleTime[2] - saved_cycleTime[1],
                                  saved_cycleTime[2] - saved_cycleTime[0]]

            print
            " "
            print
            "End of iteration No.", x + 1
            print
            "=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-="
            print
            " "
        ...
        print
        'saved Pushdata', saved_pushData
        for savedData in saved_pushData:
            asp_data = savedData.split('|')[0]
            asp_data += "," + str(initWeight) + "," + str(finalWeight) + "," + str(weightLoss) + "," + str(
                weightLoss_Ratio)
            push_data(asp_data, filename)
            dsp_data = savedData.split('|')[1]
            dsp_data += "," + str(initWeight) + "," + str(finalWeight) + "," + str(weightLoss) + "," + str(
                weightLoss_Ratio)
            push_data(dsp_data, filename)

        avoidInpErr.warningBeep()
        print
        "Loop done, data stored at: ", filename
...
def nimbang_increment_1ml(*inputs1):
    ...
    inputs2 = avoidInpErr.reInput("input please : tip, iter_per_tip, PickPos >> ",
                                  avoidInpErr.test_inputNimbangIncrement1ml)
    ...
    "====weighing properties===="
    print
    "List Volume :", list_vol
    print
    "Total Increment Pipetting:", len(list_vol)
    print
    "Tip required: ", int(iter_per_tip) * int((len(list_vol)))
    print
    "tip:", tip, "| iter per tip:", iter_per_tip, "| Initial Pickpos:", initPickPos
    print
    "Split Files:", splitStat
    print
    "==========================="
    print
    "[NOTES] ALWAYS REFILL YOUR SELECTED PICKPOS ON CADDY EVERYTIME THE VOLUME IS CHANGING!"
    ...
    if splitStat:
        # asp
        filename1 = 'grav\Gravimetric vol_' + '_asp_' + str(list_vol[0]) + '_to_' + str(
            list_vol[-1]) + '_' + time.strftime('%Y%m%d%H%M%S', time.localtime(time.time())) + '.csv'
        push_data(
            "Dates,Temp,Liquid,Tip,Type,Operation,CommandedVol,Weight,Density,CalcVol,Stable_Time_ms,Pip_Time_ms,Half_Cycle_Time_ms,Full_Cycle_time_ms,Init_Weight,Final_Weight,WeightLoss,WeightLoss_Ratio,Total_time_ms,Alc_time_ms,Sampling_vol,Total_sampling,Last_flow,P1_off,P2_off,Res_vacuum,Pipetting_done,Selected_p2,Tau_time,Pressure_peak,Avg_window,Slope_window,Threshold,Sensed_vol,Log Files",
            filename1)
        # dsp
        filename2 = 'grav\Gravimetric vol_' + '_dsp_' + str(list_vol[0]) + '_to_' + str(
            list_vol[-1]) + '_' + time.strftime('%Y%m%d%H%M%S', time.localtime(time.time())) + '.csv'
        push_data(
            "Dates,Temp,Liquid,Tip,Type,Operation,CommandedVol,Weight,Density,CalcVol,Stable_Time_ms,Pip_Time_ms,Half_Cycle_Time_ms,Full_Cycle_time_ms,Init_Weight,Final_Weight,WeightLoss,WeightLoss_Ratio,Total_time_ms,Alc_time_ms,Sampling_vol,Total_sampling,Last_flow,P1_off,P2_off,Res_vacuum,Pipetting_done,Selected_p2,Tau_time,Pressure_peak,Avg_window,Slope_window,Threshold,Sensed_vol,Log Files",
            filename2)
    else:
        filename = 'grav\Gravimetric vol_' + str(list_vol[0]) + '_to_' + str(list_vol[-1]) + '_' + time.strftime(
            '%Y%m%d%H%M%S', time.localtime(time.time())) + '.csv'
        push_data(
            "Dates,Temp,Liquid,Tip,Type,Operation,CommandedVol,Weight,Density,CalcVol,Stable_Time_ms,Pip_Time_ms,Half_Cycle_Time_ms,Full_Cycle_time_ms,Init_Weight,Final_Weight,WeightLoss,WeightLoss_Ratio,Total_time_ms,Alc_time_ms,Sampling_vol,Total_sampling,Last_flow,P1_off,P2_off,Res_vacuum,Pipetting_done,Selected_p2,Tau_time,Pressure_peak,Avg_window,Slope_window,Threshold,Sensed_vol,Log Files",
            filename)
    ...
    for i in enumerate(list_vol):
        ...
        for x in range(iter):
            ...
            "=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-="
            print
            "Start of iteration No.", x + 1, "of volume", i[1], "uL | vol no.", i[0] + 1, "of", len(list_vol)
            print
            " "
            ...
            if picktipstat[0]:
                ...
                if splitStat:
                    string_aspPushData += dates + "," + str(temp) + "," + liquid + ",P" + str(
                        tip) + ",asp," + operation + "," + str(vol) + "," + str(weight_asp) + "," + str(
                        density) + "," + str(calcvol) + "," + str(stableTime) + "," + str(pipetting_time) + "," + str(
                        Half_Cycle_Time_ms)
                else:
                ...
                "=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-="
                print
                "Mid of Iteration No.", x + 1, "of volume", i[1], "uL | vol no.", i[0] + 1, "of", len(list_vol)
                print
                "=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-="
                print
                " "
                ...
                if x == iter - 1:
                    finalWeight = weight2
                    if splitStat:
                        string_dspPushData += dates + "," + str(temp) + "," + liquid + ",P" + str(
                            tip) + ",dsp," + operation + "," + str(vol) + "," + str(weight_dsp) + "," + str(
                            density) + "," + str(calcvol) + "," + str(stableTime) + "," + str(
                            pipetting_time) + "," + str(Half_Cycle_Time_ms) + ',' + str(Full_Cycle_time_ms)
                    else:
                        string_pushData += dates + "," + str(temp) + "," + liquid + ",P" + str(
                            tip) + ",dsp," + operation + "," + str(vol) + "," + str(weight_dsp) + "," + str(
                            density) + "," + str(calcvol) + "," + str(stableTime) + "," + str(
            ...
            " "
            print
            "End of iteration No.", x + 1, "of volume", i[1], "uL | vol no.", i[0] + 1, "of", len(list_vol)
            print
            "=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-="
            print
            " "
        ...
        if splitStat:
            ...
            for data in asp_data:
                data += "," + str(initWeight) + "," + str(finalWeight) + "," + str(weightLoss) + "," + str(
                    weightLoss_Ratio)
                print
                data
        ...
        if i[0] + 1 != len(list_vol):
            ...
            " "
            print
            "[CHANGING PIPETTING VOLUME..]"
            print
            "DON'T FORGET TO REFILL YOUR SELECTED PICKPOS, ignore if it's already refilled"
            print
            " "
        else:
            print
    ...
    if splitStat:
        print
        "Loop done, data stored at: ", filename1, 'and', filename2
    else:
        print
        "Loop done, data stored at: ", filename
...
def speedMode(mode='grav'):
    if mode == 'plate':
        ...
        print
        'Pipetting Speed: Plate (Fast)'
    elif mode == 'grav' or mode == 'default':
        deck.setTravelMode('grav')
        setVelAcc_z_mode('grav')
        print
        'Pipetting Speed: Gravimetric (Slow)'
    ...
    for i in enumerate(rawMaps):  # to avoid overriding the original matrix
        ...
        for ii in i[1]:
            ...
            try:
                ...
                marks += 1
    keyMap = [i + 1 for i in range(12)];
    keyMap.insert(0, '+')
    dfMap = pd.DataFrame(maps, columns=keyMap)
    dfMap = dfMap.set_index(dfMap['+']);
    dfMap.pop('+')
...
def plate_fill(*inputs):
    ...
    if not inputs:
        ...
        if str.lower(inputs) == 'i':
            ...
            inputs = avoidInpErr.reInput('StartVol, EndVol, range >> ', avoidInpErr.test_rangeVol);
            inputs = inputs.split(',')
            list_vol = np.arange(float(inputs[0]), float(inputs[1]), float(inputs[2]))
            inputs = str.upper(
                avoidInpErr.reInput('Tip, Iter/tip, Pickpos, AspPos, DspPos >> ', avoidInpErr.test_plateFill));
            inputs = inputs.split(',')
            aspPos_state = str.lower(raw_input('Asp Pos Mode (auto/normal/lock) >> '))
            if not simulation:
                plate_fill(list_vol, int(inputs[0]), int(inputs[1]), inputs[2], inputs[3], inputs[4], aspPos_state,
                           False)
            else:
                plate_fill(list_vol, int(inputs[0]), int(inputs[1]), inputs[2], inputs[3], inputs[4], aspPos_state,
                           True)
            ...
            inputs = avoidInpErr.reInput('Desired Vol >> ');
            inputs = inputs.split(',')
            ...
            if simulation:
                ...
                inputs = str.lower(raw_input("Insert Pickpos, tip >> "));
                inputs = inputs.split(',')
    ...
    if operation:
        ...
        "Tip\t\t:", tip
        print
        "Iter/tip\t:", iter_per_tip
        print
        "Aspirate Mode\t:", aspPos_state
        print
        "AspPos:", inputs[4], '| DspPos:', inputs[5]
        print
        'Pickpos', pickpos
        volume, operation = volScaling(complete_vols, tip, 'scale');
        print
        'Volulme Calibration\t:', operation
        deck.setZeroDeckMode(tip)

        if not simulation:
            printr('Enter to continue... ');
            raw_input()
            speedMode('plate')
            starttime = time.time()
            # z picktip
    ...
    if start:
        ...
        if thread:
            ...
            while spdPlotter:
                ...
                before_t = t
                n += 1
                back_write(wraps(['{},{},{},{}'.format(t, travel, speed, c.p.read_dllt_sensor())]), filename,
                           warn=False)
        else:
...
class mainLLD():
        ...
        def __init__(self, types):
            ...
            def get_value(text=False):
                if not text:
                    ...
                    return 'Z: {}\tP1: {}\tP2: {}\tR: {}'.format(round(LLD.zero, 3), round(LLD.p1, 3), round(LLD.p2, 3),
                                                                 LLD.res)
        ...
        if manual:
            ...
            while True:
                ...
                if c.p.read_collision_sensor() < 1000:
                    ...
                    c.clear_estop()
                print
                'z: ' + str(z) + '| P1 = ' + str(p1) + ' | P2 = ' + str(p2) + ' | Diff: ' + str(
                    p1 - p2) + ' | dllt: ' + str(r) + '\r',
            LLD.set_value(z, p1, p2 + 1, r)
            print
            'LLD Set >>', LLD.get_value(True)
    ...
    def varCheck(x):
        ...
        for i in range(x):
            p1_t = abs(p1_avg - p1_rec[i]);
            p1_tAvg.append(p1_t / p1_avg)
            p2_t = abs(p2_avg - p2_rec[i]);
            p2_tAvg.append(p2_t / p2_avg)
            r_t = abs(r_avg - r_rec[i]);
            r_tAvg.append(r_t / r_avg)
    ...
    def findSurface(depth=-80, lld='dry', lowSpeed=False):
        ...
        print
        'Checking surfaceFound..'
        print
        'pressureCheck:\t', Dry.pressThres, c.p.read_pressure_sensor(1)
        print
        'resCheck:\t', init_res - c.PLLDConfig.resThres, c.p.read_dllt_sensor()
        if str.lower(lld) == 'dry':
...
class mainLLT():
    ...
    def run(operation='asp'):
        ...
        if str.lower(operation) == 'asp':
            ...
            if mainLLT.r2 > mainLLT.r1:
                ...
                mainLLT.Geo.init()
                mainLLT.Geo.start()
            elif c.PrereadingConfig.dlltMinDspThres < abs(mainLLT.r2 - mainLLT.r1) and (
                    c.PrereadingConfig.dlltMinThres < mainLLT.r2diff < c.PrereadingConfig.dlltMaxThres):
                mainLLT.Res.init()
                mainLLT.Res.start()
            else:
                mainLLT.Geo.init()
    ...
    def findSaturation(z, tip):
        ...
        while (z >= maxZ and not saturated):
            ...
            pos = c.p.get_motor_pos(0) / 100.0;
            zpack.append(pos)
            res = c.p.read_dllt_sensor();
            respack.append(res)
            decRate = res - respack[-2];
            decreasePack.append(decRate)
            if n > 5:
            ...
            n += 1
        if saturated: print
        'Resistance Saturated!'
        else: print
        'Resistance is not Saturated!'
    ...
    class Res():
        ...
        def init():
            mainLLT.Res.stop()
            c.p.set_dllt_pid(c.DLLTConfig.Res.kp, c.DLLTConfig.Res.ki, c.DLLTConfig.Res.kd,
...
class PvR():  # Pressure vs Resistance First Triggered
    ...
    def postRun():  # put this function after event that needs abort function is done
        ...
        PvR.triggering = True
        print
        'Waiting Press v Res of {} to be done..'.format(PvR.lastTestObj.types)
        while not PvR.trigDone:
            time.sleep(0.1)
        PvR.triggering = False
        print
        'Press V Res Finished, Winner:', PvR.getLastWinner()
...
def tip_reciprocate():
    ...
    for x in range(iters):
        ...
        if picktipstat[0]:
            ...
            print
            'initRes:', res_init, '| p1_init:', p1_init, '| p2_init:', atm_press
...
def hi_mode(chipNumber):
    ...
    time.sleep(
        1)  # delay 1 second to make sure selenoid valve is fully open/close. This is code operation without overshoot on valve voltage input.
...
def printr(*args):
    ...
    print
    cora.Fore.RED + cora.Style.BRIGHT + text + cora.Style.RESET_ALL
...
def printb(*args):
    ...
    for t, i in enumerate(args): text += str(i) + ' ' if t + 1 <= len(args) else str(i)
    print
    cora.Fore.CYAN + cora.Style.BRIGHT + text + cora.Style.RESET_ALL
...
def printg(*args):
    ...
    for t, i in enumerate(args): text += str(i) + ' ' if t + 1 <= len(args) else str(i)
    print
    cora.Fore.GREEN + cora.Style.BRIGHT + text + cora.Style.RESET_ALL
...
def printy(*args):
    ...
    for t, i in enumerate(args): text += str(i) + ' ' if t + 1 <= len(args) else str(i)
    print
    cora.Fore.YELLOW + cora.Style.BRIGHT + text + cora.Style.RESET_ALL
